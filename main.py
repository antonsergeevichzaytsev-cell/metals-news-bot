...
⚠️ 19.08.2026: НЕ АКТИВНО. Railway-миграция была откачена 16.08.2026
(см. memory/git history) — этот файл и api.py/seed.py остались в
репозитории как код, но ничем не запускаются. Основной путь исполнения
сейчас — GitHub Actions (.github/workflows/*.yml), как было до
Railway-эксперимента. Список JOBS ниже устарел вдвойне: помимо того,
что main.py не выполняется, он ещё и ссылается на pipeline_sync и
account_watch — оба модуля 19.08.2026 переехали в отдельный репозиторий
metals-outreach-sync и в этом репо больше не существуют.

Если Railway-путь понадобится снова — сверить JOBS с актуальным
.github/workflows/*.yml перед реактивацией, список ниже не обновлялся
с момента отката.
...
JOBS = [
    ...
    # 19.08.2026: pipeline_sync и account_watch убраны отсюда — оба
    # переехали в metals-outreach-sync, этих модулей больше нет в этом
    # репозитории. Если Railway-путь реактивируется когда-либо, их
    # нужно либо вернуть отдельным сервисом на тот репо, либо не
    # трогать здесь вообще.
